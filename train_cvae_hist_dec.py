...
from models.cvae_hist_dec import CVAEWrapper_hist_dec 
...
net = CVAEWrapper_hist_dec(cfg=args)
...
##### ------ warm-up ------- #####
avg_recons, avg_kl, avg_root, avg_kl_prior, avg_seam = 0., 0., 0., 0., 0.
for nb_iter in range(1, args.warm_up_iter+1):
    ...
    for fwd_idx in range(3):
        ...
        loss = loss_motion + loss_kl * args.kl_loss + kl_prior * args.kl_prior_loss + args.root_loss * loss_root + args.vel_loss * loss_vel + args.acc_loss * loss_acc + args.seam_loss * loss_seam
    ...
    if nb_iter % args.print_iter ==  0 :
        ...
        avg_kl_prior /= args.print_iter
        avg_seam /= args.print_iter
        logger.info(f"Warmup. Iter {nb_iter} :  lr {current_lr:.5f} \t Recons.  {avg_recons:.5f} \t KL. {avg_kl:.5f} \t Root. {avg_root:.5f} \t KL_prior. {avg_kl_prior:.5f} \t Seam. {avg_seam:.5f}")
...
if args.num_gpus > 1:
    best_iter, best_mpjpe, writer, logger = eval_trans.evaluation_cvae_multi(args.out_dir, os.path.join(args.out_dir, str(nb_iter)), val_loader, net.module, logger, writer, 0, best_iter=0, best_mpjpe=1000, device=comp_device)
else:
    best_iter, best_mpjpe, writer, logger = eval_trans.evaluation_cvae_multi(args.out_dir, os.path.join(args.out_dir, str(nb_iter)), val_loader, net, logger, writer, 0, best_iter=0, best_mpjpe=1000, device=comp_device)
...
for nb_iter in range(1, args.total_iter + 1):
    ...
    if nb_iter % args.print_iter ==  0 :
        ...
        writer.add_scalar('./Train/KL_prior', avg_kl_prior, nb_iter)
        writer.add_scalar('./Train/Seam_loss', avg_seam, nb_iter)
        writer.add_scalar('./Train/LR', current_lr, nb_iter)
        
        logger.info(f"Train. Iter {nb_iter} : \t Recons.  {avg_recons:.5f} \t KL. {avg_kl:.5f} \t Root. {avg_root:.5f} \t KL_prior. {avg_kl_prior:.5f} \t Seam. {avg_seam:.5f}")
        
        avg_recons, avg_kl, avg_root, avg_kl_prior, avg_seam = 0., 0., 0., 0., 0.

    if nb_iter % args.eval_iter==0:
        if args.num_gpus > 1:
            best_iter, best_mpjpe, writer, logger = eval_trans.evaluation_cvae_multi(args.out_dir, os.path.join(args.out_dir, str(nb_iter)), val_loader, net.module, logger, writer, nb_iter, best_iter, best_mpjpe, device=comp_device)
        else:
            best_iter, best_mpjpe, writer, logger = eval_trans.evaluation_cvae_multi(args.out_dir, os.path.join(args.out_dir, str(nb_iter)), val_loader, net, logger, writer, nb_iter, best_iter, best_mpjpe, device=comp_device)
