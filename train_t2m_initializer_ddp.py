...
from dataloader import dataset_TM_train_initializer as dataset_train_init
from dataloader import dataset_eval_t2m_initializer as dataset_eval_t2m
from dataloader.dataset_TM_train_initializer import cycle
...
comp_device = torch.device('cuda', local_rank) if use_cuda else torch.device('cpu')
...
if rank == 0:
    ...
    logger = logging.getLogger('Exp-Worker')
    writer = None

##### ---- Dataloader ---- #####
train_dataset = dataset_train_init.Text2MotionDataset(args.dataname, unit_length=args.unit_length, latent_dir=args.latent_dir, window_size=args.window_size, split='train', max_text_len=32)
...
# Validation loader (evaluate on rank 0 only)
...
while nb_iter <= args.total_iter:
    if world_size > 1 and isinstance(train_sampler, DistributedSampler):
    ...
    if nb_iter % args.eval_iter == 0 and rank == 0:
        ...
        if best_fid < prev_best_fid_local:
            save_dict = {
                'denoiser': (diffusion.module.state_dict() if isinstance(diffusion, DDP) else diffusion.state_dict()),
                'iter': nb_iter,
                'best_fid': best_fid,
                'best_top1': best_top1,
                'best_top2': best_top2,
