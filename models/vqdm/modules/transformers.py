...
class AdaLayerNorm_MLP(nn.Module):
    def __init__(self, n_embd, condition_dim, emb_type="adalayernorm_mlp"):
        super().__init__()
        # actually, diffusion step stands for num_actions
        # thus, this emb is a linear mapping from condition embedding to transformer input query
        emb = torch.nn.Parameter(torch.randn((condition_dim, n_embd)))
        self.register_parameter('emb', emb.float())
        # self.emb = nn.Embedding(condition_dim, n_embd)
        self.silu = nn.SiLU()
        self.linear = nn.Linear(n_embd, n_embd*2)
        self.layernorm = nn.LayerNorm(n_embd, elementwise_affine=False)

    def forward(self, x, timestep):
        emb = self.linear(self.silu(timestep @ self.emb)).unsqueeze(1)
        scale, shift = torch.chunk(emb, 2, dim=2)
        x = self.layernorm(x) * (1 + scale) + shift
        return x
...
class Block(nn.Module):
    ...
    def __init__(self,
        ...
        if attn_type in ['self', 'selfcondition']:
            ...
            if attn_type == 'selfcondition':
                if class_type == 'adalayernorm':
                    self.ln2 = AdaLayerNorm(n_embd, class_number, class_type)
                elif class_type == 'adalayernorm_mlp':
                    self.ln2 = AdaLayerNorm_MLP(n_embd, condition_dim, class_type)
                else:
        ...
        if mlp_type == 'conv_mlp':
            self.mlp = Conv1D_MLP(n_embd, mlp_hidden_times, act, resid_pdrop)
        else:
    ...
    def forward(self, x, encoder_output, timestep, mask=None):    
        if self.attn_type == "selfcross":
            ...
            x = x + a
            x = x + self.mlp(self.ln2(x, encoder_output))   # only one really use encoder_output
            return x, att
        else:  # 'self'
            a, att = self.attn(self.ln1(x, timestep), encoder_output, mask=mask)
            x = x + a 
...
class Conv1D_MLP(nn.Module):
    def __init__(self, n_embd, mlp_hidden_times, act, resid_pdrop):
        super().__init__()
        self.conv1 = nn.Conv1d(in_channels=n_embd, out_channels=int(mlp_hidden_times * n_embd), kernel_size=3, stride=1, padding=1)
        self.act = act
        self.conv2 = nn.Conv1d(in_channels=int(mlp_hidden_times * n_embd), out_channels=n_embd, kernel_size=3, stride=1, padding=1)
        self.dropout = nn.Dropout(resid_pdrop)
...
class Text2ImageTransformer(nn.Module):
    def __init__(
        ...
        self.content_emb = MotionMaskEmbedding(num_classes, content_seq_len, n_embd)
