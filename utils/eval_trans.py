...
# Multi-GPU evaluation of Causal TAE (training time)
@torch.no_grad()        
def evaluation_vqdm_vae(out_dir, vis_dir, val_loader, net, logger, writer, nb_iter, best_iter, best_mpjpe, draw = True, save = True, savegif = True, device=torch.device('cuda'), accelerator=None, evaluator=None): 
    net.eval()
    nb_sample = 0
    ...
    for batch in val_loader:
        ...
        mask = torch.arange(seq, device=device).unsqueeze(0) < lengths.unsqueeze(1)
        num_joints = 22
        pred_pose_eval = torch.zeros((bs, seq, motion.shape[-1])).to(device)
        for i in range(bs):
            pose = val_loader.dataset.inv_transform(motion[i:i+1, :m_length[i], :].detach().cpu().numpy())
            pose_xyz = recover_from_local_position(pose.squeeze(0), num_joints)
            pred_pose, _, _ = net(motion[i:i+1, :m_length[i]], mask[i:i+1, :m_length[i]])
            pred_pose_eval[i:i+1,:m_length[i],:] = pred_pose
...
# Multi-GPU evaluation of Causal TAE (training time)
@torch.no_grad()        
def evaluation_motionprimitive_multi(out_dir, vis_dir, val_loader, net, logger, writer, nb_iter, best_iter, best_mpjpe, draw = True, save = True, savegif = True, device=torch.device('cuda'), accelerator=None, evaluator=None): 
    net.eval()
    nb_sample = 0
    ...
    for batch in val_loader:
        ...
        pred_pose_eval = torch.zeros((bs, seq, motion.shape[-1])).to(device)
        pred_pose = net.forward_long(motion)

        for i in range(bs):
            pose = val_loader.dataset.inv_transform(motion[i:i+1, :m_length[i], :].detach().cpu().numpy())
            pose_xyz = recover_from_local_position(pose.squeeze(0), num_joints)

            pred_pose_eval[i:i+1,:m_length[i],:] = pred_pose[i:i+1, :m_length[i]]

            if accelerator is None or accelerator.is_main_process:
                pred_denorm = val_loader.dataset.inv_transform(pred_pose_eval[i:i+1, :m_length[i]].detach().cpu().numpy())
                pred_xyz = recover_from_local_position(pred_denorm.squeeze(0), num_joints)
                pred_xyz = torch.from_numpy(pred_xyz).float().to(device)
                pose_xyz = torch.from_numpy(pose_xyz).float().to(device)
                mpjpe += torch.sum(calculate_mpjpe(pose_xyz[:, :m_length[i]].squeeze(), pred_xyz[:, :m_length[i]].squeeze()))
...
# Multi-GPU evaluation of Causal TAE (training time)
@torch.no_grad()        
def evaluation_cvae_multi(out_dir, vis_dir, val_loader, net, logger, writer, nb_iter, best_iter, best_mpjpe, draw = True, save = True, savegif = True, device=torch.device('cuda'), accelerator=None, evaluator=None): 
    net.eval()
    nb_sample = 0
...
# Single-GPU evaluation of text to motion model (test time)：
@torch.no_grad()                
def evaluation_transformer_272_single_no_latent(
    val_loader,
    denoiser,
    logger,
    evaluator,
    ...
    for batch in tqdm(val_loader, desc='Evaluating'):
        ...
        # Batched generation: use max target length in batch, then slice
        motion_length_max = int(m_length.max().item()) if torch.is_tensor(m_length) else int(max(m_length))
        pred_pose = denoiser.sample(caption_enc, caption_enc_len, motion_length=motion_length_max)
        # pred_pose = latent_model.forward_decoder(index_motion)  # [B, T, D]
        # cur_len = pred_pose.shape[1]
        # fill_len = min(cur_len, seq)
        # pred_len = torch.full((bs,), fill_len, dtype=torch.long, device=device)
...
# Single-GPU evaluation of text to motion model (test time)：
@torch.no_grad()                
def evaluation_transformer_272_roll(
    val_loader,
    latent_model,
    denoiser,
    logger,
...
# Single-GPU evaluation of text to motion model (test time)：
@torch.no_grad()                
def evaluation_transformer_272_roll_no_latent(
    val_loader,
    denoiser,
    logger,
    evaluator,
    ...
    for batch in tqdm(val_loader, desc='Evaluating'):
        ...
        pred_pose = denoiser.stream_rollout(pose[:, :denoiser.W], caption_enc, caption_enc_len, motion_length_max)
        # pred_pose = latent_model.forward_decoder(index_motion)  # [B, T, D]
        # cur_len = pred_pose.shape[1]
        # fill_len = min(cur_len, seq)
        # pred_len = torch.full((bs,), fill_len, dtype=torch.long, device=device)
...
def evaluation_vqdm(
    ...
    for batch in tqdm(val_loader, desc='Evaluating'):
        ...
        text = list(text)
        motion_length_max = int(lengths.max().item())
        token_length = max(1, (motion_length_max + unit_length - 1) // unit_length)
        mask = torch.arange(seq, device=device).unsqueeze(0) < ((lengths//unit_length).unsqueeze(1))
        cond = {'y': {'text': text, 'mask': mask[:, :token_length]}}
        cond['y'] = {key: val.to(device) if torch.is_tensor(val) else val for key, val in cond['y'].items()}

        # Batched generation: sample latent tokens then decode to motion
        model_out = denoiser.sample((bs, token_length), cond, device, filter_ratio=0)
        index_motion = model_out['content_token']
        pred_pose = vqvae.forward_decoder(index_motion)  # [B, T, D]
        pred_len = torch.minimum(lengths, torch.full_like(lengths, pred_pose.shape[1]))

        for k in range(bs):
            length_k = int(pred_len[k].item())
            if length_k > 0:
                pred_pose_eval[k:k+1, :length_k] = pred_pose[k:k+1, :length_k]
