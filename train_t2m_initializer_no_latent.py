...
from utils.eval_trans import evaluation_transformer_272_single_no_latent
...
while nb_iter <= args.total_iter:
    ...
    m_tokens, m_tokens_len, feat_text, feat_text_len = m_tokens.to(comp_device).to(torch.float32), m_tokens_len.to(comp_device), feat_text.to(comp_device).to(torch.float32), feat_text_len.to(comp_device)
    ...
    if nb_iter % args.eval_iter == 0:
        ...
        eval_vis_dir = os.path.join(args.out_dir, 'eval_vis', str(nb_iter))
        os.makedirs(eval_vis_dir, exist_ok=True)
        best_fid, best_div, best_top1, best_top2, best_top3, best_matching, logger = evaluation_transformer_272_single_no_latent(
            val_loader,
            diffusion,
            logger,
            evaluator,
