...
import torch
from stablemofusion.utils.kinematics import HybrIKJointsToRotmat, HybrIKJointsToRotmat_Tensor
from stablemofusion.utils.transforms import matrix_to_axis_angle
...
def footskate_clean(pred_motions, opt, epoch, underpressure_model, model, JOINT_NAMES):
    try:
        from UnderPressure.demo import retarget_to_underpressure, Skeletons, FRAMERATE, TOPOLOGY
        from UnderPressure import util, anim
        from UnderPressure.data import Contacts
        from UnderPressure.metrics import MSLE
    except Exception as e:
        raise ImportError(
            \"footskate_cleanup=True requires the external 'UnderPressure' package on PYTHONPATH. \"
            \"Follow the StableMoFusion README UnderPressure setup steps.\"
        ) from e
    """
