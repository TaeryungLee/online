...
import math
import tqdm
from stablemofusion.utils.footskate_clean import footskate_clean
from stablemofusion.models import vGRFmodel
import os
from stablemofusion.utils.paramUtil import HumanML3D_JOINT_NAMES, KIT_JOINT_NAMES
...
class DiffusePipeline(object):
    ...
    def __init__(self, opt, model, diffuser_name, num_inference_steps, device, torch_dtype=torch.float16):
        ...
        from stablemofusion.utils.paths import get_diffuser_params_path
        with open(get_diffuser_params_path(), 'r') as yaml_file:
    ...
    def load_footskate_cleanup_model(self):
        """Optionally load footskate cleanup models.
        Requires the external UnderPressure codebase to be importable as `UnderPressure`.
        """
        try:
            from UnderPressure import models as underpressure_models
        except Exception as e:
            raise ImportError(
                "footskate_cleanup=True requires the external 'UnderPressure' package "
                "to be available on PYTHONPATH. See StableMoFusion README for setup."
            ) from e
            ...
            underpressure_model = underpressure_models.DeepNetwork(state_dict=torch.load(underpressure_model_path)["model"]).eval()
